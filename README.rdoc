...
Let's say you need to monitor some services. You can use the create_resources api
to specify the services you need. Services usually consist of a log entry and its
...
If you remove a service you need to do so manually.
...
If you want to show off all the services, you would do that like so.
...
There is no way to include all the available services automatically. It was a 
...
Note that the destinations and their services need unique names. For the destinations 
their name is also the hostname or ip of the remote syslog server.
...
With this setup Messages from the syslog-ng service will be logged to both servers as 
...
I accept pull requests containing only spec. If you found some exotic use case that work 
nicely on your infrastructure please consider documenting it in a spec case so future 
updates of this module don't introduce breakage.

The follwing commands come in handy when hacking with this module.
